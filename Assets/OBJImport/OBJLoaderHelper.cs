...
namespace Dummiesman
{
    public static class OBJLoaderHelper
    {
        ...
        {
            Shader shader = Shader.Find("Universal Render Pipeline/Lit"); // URP default
            if (shader == null)
            {
                shader = Shader.Find("HDRP/Lit"); // HDRP fallback
            }
            if (shader == null)
            {
            ...
            if (shader == null)
            {
                Debug.LogError("No compatible fallback shader found!");
                return null;
            }

            return new Material(shader) { name = "Fallback_Material" };
        }
